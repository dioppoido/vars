    ...
    if(req.session.user){
        if(req.query.eventid){
            if(req.session.user.admin) {
                ...
                res.render('errorconfirmation.ejs',{msg:'一般ユーザはこの機能を利用できません。',url:'/eventtop?eventid='+req.query.eventid});
            }
        }else{
            res.render('errorconfirmation.ejs',{msg:'イベントIDが存在しません',url:'/eventlist'});
        }
    ...
    if(req.session.user){
        if(req.session.user.admin) {
            ...
            res.render('confirmation.ejs', {
                msg: 'イベント情報を変更しました。',
                url: '/eventcontrol/eventsetting?eventid=' + eventid
            });
        }else{
            res.render('errorconfirmation.ejs',{msg:'一般ユーザはこの機能を利用できません。',url:'/eventtop?eventid='+req.query.eventid});
        }
                ...
                getEvent.getEvent(eventid).then(function (eventdata) {
                    getVote.getVotejson({Eventid: eventid}).then(function (vote) {
                        ...
                        res.render('errorconfirmation.ejs', {
                            msg: '投票データがありません。',
                            url: '/eventcontrol?eventid=' + eventid
                        });
    ...
    if (req.session.user) {
        if (req.session.user.admin) {
            ...
            res.render('errorconfirmation.ejs', {
                ...
                url: '/eventtop?eventid=' + req.query.eventid
            });
        }
    } else {
        res.redirect('/');
                  ...
                  getEvent.getEvent(eventid).then(function (eventdata) {
                      ...
                      async.eachSeries(eventdata[0].Order, function (order, callback) {
                          getTeam.getTeamjson({Teamid: order}).then(function (team) {
                              ...
                              res.render('errorconfirmation.ejs', {
                                  msg: msg,
                                  url: '/eventcontrol/announcesetting?eventid=' + eventid
                              });
